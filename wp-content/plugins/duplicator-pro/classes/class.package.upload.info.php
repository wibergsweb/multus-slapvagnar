...
if (!class_exists('DUP_PRO_Upload_Status'))
{
...
if (!class_exists('DUP_PRO_Package_Upload_Info'))
{
    ...
    class DUP_PRO_Package_Upload_Info
    {
        ...
        public function increase_failure_count()
        {
            ...
            DUP_PRO_U::log("failure count increasing for storage id $this->storage_id to $this->failure_count");

            if ($this->failure_count > $global->max_storage_retries)
            {
                DUP_PRO_U::log("storage $this->storage_id moving to failed");
                $this->failed = true;
            }
        }
    }
