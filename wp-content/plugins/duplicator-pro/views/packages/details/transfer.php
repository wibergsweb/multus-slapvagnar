...
<div class="transfer-panel">
	...
	<div id="step1-section" style="display:none">
		...
		<label for="files-installer"><?php echo DUP_PRO_U::__('Installer') . ' ' . DUP_PRO_Util::ByteSize($package->Installer->Size); ?> </label>
		...
		<label for="files-database"><?php echo DUP_PRO_U::__('Database') . ' ' . DUP_PRO_Util::ByteSize($package->Database->Size); ?></label> 
	</div>
				...
				foreach ($storage_list as $store) :
					...
					if ($store->id != DUP_PRO_Virtual_Storage_IDs::Default_Local)
					{
						...
						<tr class="package-row <?php echo $row_style ?>">
							<td>
								<input class="duppro-storage-input" <?php echo DUP_PRO_U::echo_disabled($is_valid == false); ?> name="_storage_ids[]" data-parsley-errors-container="#storage_error_container" <?php echo $mincheck; ?> type="checkbox" value="<?php echo $store->id; ?>" />
